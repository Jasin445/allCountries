...
export default function ErrorComponent({err, noHead}: any){
    ...
    <div className={`flex gap-9 w-full justify-center items-center flex-col h-[60vh] max-sm:h-[70vh]`}>
        ...
        { location.pathname === "/countries" ? <Link to="/countries"  className={`px-3 py-[3px] bg-slate-500 rounded ${err ? 'hidden' : 'block'}`}>Reload</Link> : <Link to={'/countries'} className="px-3 py-[3px] bg-slate-500 rounded">Reload</Link> }
    </div>
    </>
    )
